...
class CompositionAgent(BaseAgent):
    """
    An agent that analyzes the composition and layout of images.
    """
    ...
    async def _process_message_impl(self, message: AgentMessage) -> AgentMessage:
        ...
        content = [{
            "type": "text",
            "text": "Analyze the composition of the following images. Describe the layout, spacing, and how elements are arranged. Also, consider the original prompt provided by the user and describe how the images relate to it. Your analysis will be used to generate a Midjourney prompt."
        },
        {
            "type": "text",
            "text": f"Original prompt for context: '{original_prompt}'"
        }]
        ...
        try:
            ...
            return self._create_error_response(message.sender_id, f"Failed to analyze composition with OpenAI: {e}")

        return AgentMessage(
            sender_id=self.agent_id,
            recipient_id=message.sender_id,
            content={"composition_analysis": analysis},
