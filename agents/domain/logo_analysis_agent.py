...
class LogoAnalysisAgent(BaseAgent):
    """
    An agent that analyzes images specifically for logos and distinct graphical elements.
    """
    ...
    async def _process_message_impl(self, message: AgentMessage) -> AgentMessage:
        ...
        content = [{
            "type": "text",
            "text": "Analyze the following images and provide a detailed description of any logos or distinct graphical elements. If there are no logos, state that clearly. Your analysis will be used to generate a Midjourney prompt."
        }]
        ...
        try:
            ...
            return self._create_error_response(message.sender_id, f"Failed to analyze images with OpenAI: {e}")

        return AgentMessage(
            sender_id=self.agent_id,
            recipient_id=message.sender_id,
            content={"logo_analysis": analysis},
