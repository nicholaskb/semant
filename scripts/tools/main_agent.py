...
from agents.utils.email_integration import send_email, send_sms
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    if result.get("status") == "sent_real":
                        ...
                        chain_of_thought.append(f"Email queued for {to_addr} (simulated)")
            except Exception as e:
                logger.error(f"Email send failed: {e}")
                chain_of_thought.append("Attempted to send email but encountered an error.")

        # Optional SMS dispatch if user explicitly requests it
        if message.lower().startswith("sms ") or message.lower().startswith("text "):
            try:
                # expected format:  sms +1234567890 your message text
                # or: text +1234567890 your message text
                parts = message.split(maxsplit=2)
                if len(parts) >= 3:
                    phone_number = parts[1]
                    body = parts[2]
                    # Include agent response in SMS
                    sms_body = f"{body}\n\nAgent: {response[:100]}"  # Truncate to fit SMS limits
                    result = send_sms(recipient_id=phone_number, body=sms_body, force_real=True)
                    if result.get("status") == "sent_real":
                        chain_of_thought.append(f"Sent SMS to {phone_number}")
                    else:
                        chain_of_thought.append(f"SMS send attempted but may have failed: {result.get('status')}")
            except Exception as e:
                logger.error(f"SMS send failed: {e}")
                chain_of_thought.append("Attempted to send SMS but encountered an error.")
