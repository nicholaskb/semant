...
@pytest.mark.asyncio
async def test_pan_and_outpaint_endpoints(monkeypatch, client):
    ...
    async def dummy_post(url, headers=None, **kwargs):
        ...
        if tt == "pan":
            return DummyResponse(200, {"data": {"task_id": "pan-1"}})
        if tt == "outpaint":
            return DummyResponse(200, {"data": {"task_id": "op-1"}})
    ...
    # PAN
    r1 = client.post("/api/midjourney/pan", json={"origin_task_id": "t-1", "direction": "right"})
    assert r1.status_code == 200, r1.text
    d1 = r1.json()
    assert d1["task_id"] == "pan-1"
    assert d1["gcs_url"].startswith("https://storage.googleapis.com/")

    # OUTPAINT
    r2 = client.post("/api/midjourney/outpaint", json={"image_url": "https://img", "prompt": "extend"})
    assert r2.status_code == 200, r2.text
    d2 = r2.json()
    assert d2["task_id"] == "op-1"
    assert d2["gcs_url"].startswith("https://storage.googleapis.com/")
