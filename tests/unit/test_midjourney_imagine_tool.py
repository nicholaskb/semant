...
@pytest.mark.asyncio
async def test_imagine_tool_runs_and_logs(monkeypatch):
    ...
    async def dummy_post(url, headers=None, **kwargs):
        return DummyResponse(200, {"data": {"task_id": "xyz", "output": {"image_url": "https://img"}}})
    ...
    tool = ImagineTool()
    resp = await tool.run(prompt="test", model_version="v7", oref="u", ow=5)
    assert resp["data"]["task_id"] == "xyz"
